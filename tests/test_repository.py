...
class TestGetConcertIdsByKopisIds:
    def _run(self, kopis_ids, mock_session):
        with patch("db.repository.get_session") as mock_get_session:
            mock_get_session.return_value.__enter__ = MagicMock(return_value=mock_session)
            mock_get_session.return_value.__exit__ = MagicMock(return_value=False)
            return get_concert_ids_by_kopis_ids(kopis_ids)

    def test_returns_empty_dict_without_db_call_for_empty_list(self):
        """kopis_ids가 빈 리스트면 DB 조회 없이 {} 반환해야 한다."""
        mock_session = MagicMock()
        with patch("db.repository.get_session") as mock_get_session:
            result = get_concert_ids_by_kopis_ids([])

        mock_get_session.assert_not_called()
        mock_session.execute.assert_not_called()
        assert result == {}

    def test_returns_kopis_id_to_concert_id_dict(self):
        """조회된 (kopis_id, id) 행들을 {kopis_id: concert_id} dict로 반환해야 한다."""
        mock_session = MagicMock()
        mock_session.execute.return_value.fetchall.return_value = [
            ("PF001", 10),
            ("PF002", 11),
        ]
        result = self._run(["PF001", "PF002"], mock_session)

        assert result == {"PF001": 10, "PF002": 11}
